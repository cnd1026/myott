...
function safeObservabilityExclusionReason(item = {}) {
  const reason = String(item.exclusionReason || "").trim().toLowerCase();
  ...
  if (acceptedReasons.has(reason)) return reason;
  if (item.hardFilterStatus?.contentType === "fail") return "content-type-mismatch";
  if (item.countryValidation === "mismatch") return "country-mismatch";
  if (item.genreMatched === false) return "semantic-genre-insufficient";
  if (item.pass === false) return "hard-filter-failed";
  return "other-product-exclusion";
}
...
function emitCurrentProductObservability({
  ...
  for (const item of [...finalized.results, ...finalized.relaxedResults]) {
    const candidateId = observabilityCandidateId(item);
    if (candidateId && !selectedById.has(candidateId)) selectedById.set(candidateId, item);
  }
  const excludedById = new Map();
  for (const item of finalized.exclusions) {
    const candidateId = observabilityCandidateId(item);
    if (candidateId && !selectedById.has(candidateId) && !excludedById.has(candidateId)) {
      excludedById.set(candidateId, item);
    }
  }
  const stageSummaries = [
    ...
    },
    {
      ...
      outputCount: Number(finalized.diagnostics.classifiedCount || 0),
      excludedCount: Number(finalized.diagnostics.hardFilterExclusions || 0),
    },
    {
      stage: "ranking",
      inputCount: Number(finalized.diagnostics.classifiedCount || 0),
      outputCount: selectedById.size,
      excludedCount: excludedById.size,
    },
    {
      stage: "final-selection",
      inputCount: selectedById.size + excludedById.size,
      outputCount: selectedById.size,
      excludedCount: excludedById.size,
    },
  ];
  ...
  let rank = 1;
  for (const [candidateId] of selectedById) {
  ...
  for (const [candidateId, item] of excludedById) {
    emitTmdbObservabilityEvent(session, "candidate-decision", {
      candidateId,
      stage: "final-selection",
      decision: "excluded",
      reason: safeObservabilityExclusionReason(item),
      rank: null,
    });
  }
