...
function discoverPathsForTypes(contentTypes = []) {
  const types = contentTypes.length ? contentTypes : ["movie", "drama", "animation"];
  const paths = new Set();

  if (types.includes("movie") || types.includes("animation")) paths.add("movie");
  if (types.includes("drama")) paths.add("tv");
...
export async function discoverTmdb({ filters = [], contentTypes = [], limit = TMDB_RECOMMENDATION_LIMIT } = {}) {
  ...
  const onlyAnimationSelected = contentTypes.length === 1 && contentTypes.includes("animation");
  const genreIds = [...new Set([...genreIdsFromFilters(filters), ...(onlyAnimationSelected ? [16] : [])])];
  const country = countryFromFilters(filters);
  const results = [];

  for (const mediaType of discoverPathsForTypes(contentTypes)) {
    const params = {
      page: "1",
