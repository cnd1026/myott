...
function detailCandidateOrder(items = [], filters = []) {
  ...
  const selectedValuesForItem = (item) => [...new Set([
    ...selectedValues,
    ...(item.crossMediaSeedGenreValues || []),
  ])];
...
async function fetchSeedDiscoverSupplement(
  ...
  const selectedGenreValues = selectedTaxonomyFilters(filters);
  const transferredGenreValues = transferableSeedGenreValues(record.seed.genreIds || [], request.mediaType);
  const targetGenreValues = selectedGenreValues.length ? selectedGenreValues : transferredGenreValues;
  const selectedGenreIds = genreIdsForFilters(targetGenreValues, request.mediaType);
  const transferredGenreIds = transferableSeedGenreIds(record.seed.genreIds || [], request.mediaType);
  const targetGenreIds = [...new Set((selectedGenreIds.length ? selectedGenreIds : transferredGenreIds).map(Number))]
    .filter(Number.isFinite)
    .filter((id) => id !== 16);
  if (!targetGenreIds.length && !request.animationOnly) return [];
  const genreParam = request.animationOnly
    ? targetGenreIds.length ? `16,${targetGenreIds[0]}` : "16"
    : targetGenreIds.join("|");
  ...
  if (retrievalDiagnostics) {
    const taskKey = `${source}:${record.seed.tmdbId}:${genreParam}`;
    retrievalDiagnostics.providerTotalResultsByTask[taskKey] = Number.isFinite(Number(payload.total_results))
      ? Number(payload.total_results)
      : null;
    retrievalDiagnostics.fetchedPagesByTask[taskKey] = [Number(payload.page || 1)];
  ...
  return (payload.results || []).map((item) => {
    ...
    return {
      ...
      crossMediaSeedGenreValues: crossMedia ? targetGenreValues : [],
    };
  });
}
...
export async function recommendSeedsTmdb({
  ...
  if (!candidateState.typeCoverage.covered && coverageSupplementTasks.length && context.hasTimeRemaining()) {
    await mapWithConcurrency(coverageSupplementTasks, TMDB_REQUEST_CONCURRENCY, async ({ record, request }) => {
      record.candidatePhaseAttempted = true;
      try {
        const candidates = await fetchSeedDiscoverSupplement(
          record,
          filters,
          request,
          context,
        ...
        record.phases.push(`cross-media-discover:${request.mediaType}`);
      } catch (error) {
        record.error = error;
        if (resourceStopError(error)) {
          uniqueSeedTitles(record.inputTitles || [record.title]).forEach((title) => initiallyDeferred.add(title));
  ...
  if (!candidateState.enough && context.hasTimeRemaining() && remainingListRequests(context) > 0) {
    ...
    await mapWithConcurrency(supplementTasks, TMDB_REQUEST_CONCURRENCY, async ({ record, request }) => {
      ...
      try {
        ...
        record.candidateRequestSucceeded = true;
        successfulCandidateRequests += 1;
        record.phases.push(`discover-supplement:${request.mediaType}`);
      } catch (error) {
        record.error = error;
        if (resourceStopError(error)) {
          uniqueSeedTitles(record.inputTitles || [record.title]).forEach((title) => initiallyDeferred.add(title));
  ...
  const relationshipCandidates = enrichedCandidates.filter((item) => {
    const requiredValues = item.crossMediaSeedGenreValues || [];
    if (!requiredValues.length) return true;
    const related = candidateGenreMatchDetail(item, requiredValues).genreMatched;
    if (!related) seedRelationshipExclusions.push({ ...item, exclusionReason: "cross-media-seed-genre-mismatch" });
    return related;
  });
  const seedGenreIds = [
  ...
  return {
    ...
    diagnostics: {
      ...
      crossMediaRelationshipPassCount: relationshipCandidates.filter((item) => item.crossMediaSeedGenreValues?.length).length,
