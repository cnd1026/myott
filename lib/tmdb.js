...
const DISCOVER_FALLBACK_STAGES = [
...
function normalizeTmdbItem(item, mediaType = item.media_type, detail = item) {
  ...
  return {
    ...
    originCountries: detail.origin_country || item.origin_country || [],
...
function contentMatchesCountry(item, country = "") {
  if (!country) return true;
  return (item.originCountries || []).includes(country);
}
...
function withGenreParamForRequest(filters, request, includeGenre = true) {
...
function discoverStageParams(filters, request, country, runtimeConstraint, stage) {
  ...
  const genreParam = withGenreParamForRequest(filters, request, stage.useGenre);
...
function pushUniqueDiscoverResults(target, items, seen, stage, country = "") {
  for (const item of items) {
    if (stage.useCountry && !contentMatchesCountry(item, country)) continue;
    const key = `${item.type}-${item.tmdbId || item.title}`;
    ...
    target.push({
      ...
      reason: stage.relaxed ? RELAXED_REASON : item.reason,
    });
  }
}
...
export async function discoverTmdb({ filters = [], contentTypes = [], limit = TMDB_RECOMMENDATION_LIMIT } = {}) {
  ...
  for (const request of discoverRequestsForTypes(requestedTypes)) {
    let requestResultCount = 0;
    for (const stage of DISCOVER_FALLBACK_STAGES) {
      if (requestResultCount >= limit) break;
      const params = discoverStageParams(filters, request, country, runtimeConstraint, stage);
      const payload = await tmdbGet(`/discover/${request.mediaType}`, params);
      const normalizedResults = (payload.results || []).map((item) => normalizeTmdbItem(item, request.mediaType));
      const beforeCount = results.length;
      pushUniqueDiscoverResults(results, normalizedResults, seen, stage, country);
      requestResultCount += results.length - beforeCount;
    }
  }

  const uniqueResults = results
    .filter((item) => contentMatchesRequestedTypes(item, requestedTypes));
  const balancedResults = balanceDiscoveredResults(uniqueResults, requestedTypes).slice(0, limit);
  const enrichedResults = await enrichWatchProviders(balancedResults);
...
export async function searchTmdb(query, contentTypes = [], filters = []) {
  ...
  for (const item of recommended) {
    ...
    if (seen.has(key)) continue;
    seen.add(key);
    if (contentMatchesRequestedTypes(item, contentTypes) && contentMatchesCountry(item, country)) results.push(item);
    if (results.length >= TMDB_RECOMMENDATION_LIMIT) break;
  }

  if (country && results.length < TMDB_RECOMMENDATION_LIMIT) {
    for (const item of recommended) {
      const key = `${item.type}-${item.tmdbId || item.title}`;
      if (!contentMatchesRequestedTypes(item, contentTypes)) continue;
      if (contentMatchesCountry(item, country)) continue;
      if (results.some((result) => `${result.type}-${result.tmdbId || result.title}` === key)) continue;
      results.push({
        ...item,
        fallbackRelaxed: true,
        fallbackStage: "country",
  ...
  const enrichedResults = await enrichWatchProviders(results);
