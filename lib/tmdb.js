...
function genreIdsFromFilters(filters = []) {
  const genreMap = {
    "genre-sf": [878, 10765],
    "genre-romance": [10749],
    "genre-thriller": [53],
    "genre-animation": [16],
    "genre-action": [28, 10759],
    "genre-adventure": [12],
    "genre-comedy": [35],
    "genre-crime": [80],
    "genre-drama": [18],
...
function discoverPathsForTypes(contentTypes = []) {
  const types = normalizedContentTypes(contentTypes);
  const requestedTypes = types.length ? types : ALL_CONTENT_TYPES;
  const paths = new Set();

  if (requestedTypes.includes("movie") || requestedTypes.includes("animation")) paths.add("movie");
  if (requestedTypes.includes("drama")) paths.add("tv");
...
export async function discoverTmdb({ filters = [], contentTypes = [], limit = TMDB_RECOMMENDATION_LIMIT } = {}) {
  ...
  const focusedTypes = hasFocusedContentTypes(requestedTypes);
  const animationSelected = requestedTypes.includes("animation");
  const onlyAnimationSelected = requestedTypes.length === 1 && animationSelected;
  const genreIds = [
    ...new Set([...genreIdsFromFilters(filters), ...(onlyAnimationSelected ? [16] : [])].filter((genreId) => animationSelected || genreId !== 16)),
  ];
  const country = countryFromFilters(filters);
  const results = [];

  for (const mediaType of discoverPathsForTypes(requestedTypes)) {
    const params = {
      page: "1",
    ...
    if (focusedTypes && !animationSelected) params.without_genres = "16";
    if (country) params.with_origin_country = country;

    const payload = await tmdbGet(`/discover/${mediaType}`, params);
    results.push(...(payload.results || []).map((item) => normalizeTmdbItem(item, mediaType)));
  }
