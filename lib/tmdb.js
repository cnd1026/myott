...
function serviceName(providerName = "") {
  ...
  if (lowered.includes("amazon prime video")) return "Amazon Prime Video";
  if (lowered === "apple tv" || lowered.includes("apple tv plus")) return "Apple TV+";
  if (lowered.includes("apple tv store")) return "Apple TV Store";
...
function watchProvidersFromDetail(detail = {}) {
  ...
  const normalizeNames = (groups) => [...new Set(groups.flatMap((group) =>
    watchAvailability[group].map((provider) => serviceName(provider.name)),
  ).filter(Boolean))];
