...
작품 metadata는 `providerGenreIds`, `providerGenreNames`, `canonicalGenreValues`, `combinedGenreValues`, `semanticGenreValues`, `formatValues`, `audienceValues`, `styleValues`를 분리합니다. Provider ID는 evidence이며 그 ID에 연결된 모든 사용자 장르를 자동으로 부여하지 않습니다.

- TV `10759`는 액션·모험 combined evidence이며 액션 또는 모험 signal이 확인될 때 한쪽만 semantic specialization한다.
...
장르 Match 우선순위는 Canonical Exact, Semantic Specialized, Provider Combined, Adjacent, Relaxed 순입니다. Weight 숫자는 유지하고 Genre signal 입력의 의미만 정규화합니다. 추천 이유는 실제 semantic evidence 또는 Provider combined 한계를 반영하며 기술 ID를 노출하지 않습니다.
