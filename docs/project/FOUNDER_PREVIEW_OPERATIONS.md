...
Related Task: MYOTT-S09-OPS-001
...
Runtime 파일은 Repository 내부에 생성하지 않습니다.
...
| File | Purpose |
| --- | --- |
| `state.json` | 관리 중인 Launcher/Listener, Repository, 시작 Commit, Log 경로 |
| `lifecycle.lock` | Named Mutex 획득 중인 Action과 Owner PID 진단 |
| `last-operation.json` | 마지막 Action 결과, Exit Code, 복구/검증 세부 정보 |
| `founder-3000.out.log` | Next.js stdout |
| `founder-3000.err.log` | Next.js stderr |
...
- Listener 또는 Ancestor Command Line에 현재 Repository 절대 경로가 존재한다.
...
3. 3000-3101 Listener 조사
4. Cleanup Dry Run 진단
5. 3001-3101의 Owned MyOTT Server 정리
6. 3000 Ensure
7. Root HTTP Smoke
8. 최종 Port 상태 확인
9. Lock 해제

다른 프로그램 Listener는 유지하고 warning으로 보고합니다.
...
2. 3000 Founder Preview 재시작
3. Root HTTP 200 확인
4. Recommendation API HTTP 200 확인
5. TMDB source, fallback false, Mock 혼합 0, 결과 8개 이상 확인
6. 현재 Branch, Commit, Working Tree 기록
7. 3000 Server를 실행 상태로 유지

Finalize Restart 뒤 브라우저의 Next.js Dev Client가 다시 연결할 수 있습니다. OS 수준의 강제 Browser Refresh는 사용하지 않습니다.

---

## 9. Safe Build and Check
...
변경 Action은 Repository Path hash 기반 Windows Named Mutex를 사용합니다.

- 기본 대기 시간: 30초
- Lock 정보: `%TEMP%\myott-founder-preview\lifecycle.lock`
...
## 13. HMR and Browser Reconnection
...
```text
%TEMP%\myott-founder-preview\founder-3000.out.log
%TEMP%\myott-founder-preview\founder-3000.err.log
```
...
- Named Mutex는 같은 Windows 사용자 환경에서 동시 lifecycle mutation을 방어합니다.
