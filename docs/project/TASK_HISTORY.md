...
| --- | --- | --- | --- | --- |
| MYOTT_PHASE1_STALE_QA_EXPECTATION_DEBT_V1 | Recommendation stale QA baseline correction | `b2feb128f79b941319c40d24494245cbfdd78696` | 현재 i18n 계약에 맞게 stale 기대값 2건만 교정해 추천 회귀 기준을 274/274 PASS로 복구. Product runtime 변경 없음. | PASS / CLOSED / Founder QA not required |
| MYOTT_PHASE1_EMPTY_ERROR_RECOVERY_UX_V1 | Phase 1 recommendation error recovery UX | `4937349a012343e75f9fa6a48151ee7fa95b9869` | 오류 상태에 `조건 다시 확인하기` CTA를 추가. 기존 조건을 유지한 채 조건 영역으로 스크롤하고 `#titleInput1`에 포커스하며 자동 추천·Provider 요청은 0. Live TMDB는 실행·검증하지 않음. | PASS / CLOSED / Founder QA PASS |
| MYOTT_PHASE1_INPUT_OPTIONALITY_CLARITY_V1 | Phase 1 input optionality clarity | `ccf86fcff8e3f6a2f0d7bd4e2766a74a03473fbc` | KO `원하는 조건만 골라도 됩니다. 모든 항목을 채울 필요는 없어요.` / EN `Choose only the criteria that matter to you. You don't need to fill in every field.`를 승인. validation, request, Provider, ranking 의미 변경 없음. | PASS / CLOSED / Founder QA PASS |
...
| MYOTT_CODEX_MODE_V2_UI_SEMANTIC_RECONCILIATION_V1 | Codex Mode V2 UI semantic reconciliation | `this commit` | Founder UI snapshot과 official Ultra/Daybreak semantics를 대조해 Ultra reasoning choice와 Daybreak defensive access track을 교정. Runtime/Nd_core/Continuity 변경 없음. | local policy reconciliation complete |
...
| MYOTT_PRIVACY_CENTER_AND_ANALYTICS_CONSENT_UI_FOUNDATION_V1 | Privacy Center and Analytics consent UI foundation | `this commit` | Four-category provider-neutral view model, ko/en copy parity, allow/deny/withdraw consent transition과 effective eligibility separation을 deterministic QA로 검증. | local unmounted foundation ready / persistence·public mount·live Analytics 0 |
| MYOTT_ANALYTICS_CONSENT_ELIGIBILITY_RUNTIME_FOUNDATION_V1 | Analytics consent eligibility runtime foundation | `this commit` | Provider-neutral fail-closed privacy state, consent/jurisdiction reasoned suppression, active-request abort, ephemeral identity retirement와 future-only re-consent를 deterministic QA로 검증. | local runtime foundation ready / Product event wiring·live Analytics 0 |
...
| MYOTT_SEO_INDEXABILITY_LAUNCH_CONTRACT_V1 | SEO indexability launch architecture | `this commit` | Canonical host, preview isolation, locale URL/hreflang, robots/sitemap, noindex-origin proof, activation Gate와 rollback을 docs-only로 검증. | `INDEXABILITY_HOLD` 정본화 / runtime·Release 별도 Gate |
