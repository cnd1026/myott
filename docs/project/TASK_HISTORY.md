...
| MYOTT-S08-BUG-002 | Narrow filter progressive fallback | `이번 커밋` | 콘텐츠 타입은 hard filter로 유지하고 genre/country만 단계적으로 완화해 강한 조건에서도 결과 수를 보강. fallback 안내 Insight와 Mock QA 샘플 추가. | 완료 |
