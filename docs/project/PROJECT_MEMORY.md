...
| PM-010 | 2026-09-11 | Free-First Revenue-Triggered Expansion | `FREE_FIRST = ACTIVE`, `PRE_REVENUE_COST_POSTURE = MINIMUM_REQUIRED_SPEND_ONLY`, `CURRENT_EXPECTED_PAID_BASELINE = DOMAIN_ONLY`로 둔다. 기본 순서는 free alternative, existing infrastructure, feature scope reduction, nonessential feature disable, capability/jurisdiction defer, material trigger 이후 paid solution이다. | 가상의 미래 필요만으로 optional paid legal, infrastructure 또는 provider service를 채택하지 않고 Product 가치와 비용을 함께 보호하기 위해서다. | `LEGAL_REVIEW_REQUIRED != HIRE_LAWYER_NOW`이며 Global-first는 첫날부터 worldwide paid support나 legal certification을 뜻하지 않는다. | Active |
| PM-011 | 2026-09-11 | Stateless Global-First Phase 1 | Founder가 `KR / JP / EU / US` 범위의 stateless recommendation core를 승인했다. non-EU EEA는 out-of-scope/not-reviewed이고 Analytics, persistent guest continuity, account, guest-to-account merge, persistent personalization과 Marketing은 현재 OFF다. | 최소 데이터와 낮은 운영 복잡도로 core Product 가치를 먼저 검증하기 위해서다. | 이 상태는 Phase 1 operating policy이며 future guest/account/personalization architecture를 폐기하지 않는다. Material evidence 또는 approved business trigger가 있으면 별도 Gate로 다시 열 수 있다. | Active |
