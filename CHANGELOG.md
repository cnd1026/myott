...
- TMDB 검색/발견/Related 결과가 최종 화면에 노출되기 전에 watch provider 데이터를 확인하도록 보강했습니다.
- 실제 provider 데이터가 있을 때만 카드와 상세 레이어에 OTT 제공처를 표시하고, 없으면 `OTT 정보 확인 필요` fallback을 유지합니다.
- 선택한 OTT 옵션이 실제 제공처처럼 표시되지 않도록 selected option과 actual provider 표시 경로를 분리한 기존 안전장치를 유지했습니다.
- 추천 카드 안의 비동작 보조 CTA를 제거해 클릭 가능한 것처럼 보이는 혼선을 줄였습니다.
- 런타임 fallback 문구를 `정보 확인 필요`로 바꿔 `상세 확인`이 동작처럼 보이지 않게 정리했습니다.
- Related Picks 카드 제목은 2줄, 보조 문구는 2줄까지 자연스럽게 보이도록 카드 높이와 clamp를 조정했습니다.

### 이유

- OTT 제공처는 선택 필터가 아니라 실제 watch provider 데이터에 기반해야 사용자가 신뢰할 수 있습니다.
- 클릭되지 않는 문구가 버튼처럼 보이면 Detail Layer에서 다음 행동을 오해할 수 있습니다.
- Related Picks는 탐색 보조 영역이므로 긴 제목과 추천 문구가 레이아웃을 깨지 않으면서 충분히 읽혀야 합니다.

### 다음 작업

- Founder 로컬 TMDB 환경에서 provider region별 실제 노출 품질을 확인합니다.
- OTT deep link는 별도 Sprint에서 실제 provider 데이터 품질을 확인한 뒤 검토합니다.
